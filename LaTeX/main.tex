...
\paragraph{Question / thought process.}
I am trying to understand why the portfolio holdings process $(\varphi_t)$ is required to be $\mathcal F_{t-1}$-measurable. 
My intuition is:
``the atoms relevant for $\varphi_t$ are already present in the previous sigma-algebra, so we do not need to wait until time $t$ for $\varphi_t$ to be measurable.'' 
In contrast, for a price process $(S_t)$ the observation at time $t$ provides genuinely new information, so typically $S_t$ is $\mathcal F_t$-measurable but not $\mathcal F_{t-1}$-measurable. 
If the filtration $(\mathcal F_t)$ is generated by the risky asset prices, then holdings are taken with respect to that same filtration with a one-step lag (i.e.\ $\varphi_t$ is $\mathcal F_{t-1}$-measurable). 
I think of $\mathcal F$ as being generated by prices rather than by $\varphi$.

\paragraph{Clean statement / answer.}
The precise meaning of ``$\varphi_t$ is $\mathcal F_{t-1}$-measurable'' is:
\[
\forall\ \text{Borel }B\subseteq \mathbb R^{d+1},\qquad
\{\omega:\varphi_t(\omega)\in B\}\in \mathcal F_{t-1}.
\]
Equivalently, $\varphi_t$ is \emph{constant on each atom of $\mathcal F_{t-1}$}; i.e.\ two scenarios $\omega,\omega'$ that are indistinguishable at time $t-1$ must lead to the same holdings $\varphi_t(\omega)=\varphi_t(\omega')$.
This is exactly the \emph{predictability} (``no peeking'') condition: the holdings used over the interval $(t-1,t]$ must be chosen using only the information available at time $t-1$.

If the filtration is generated by prices, e.g.\ $\mathcal F_t=\sigma(S_0,\dots,S_t)$, then predictability can be written as the existence of a measurable decision rule $g_t$ such that
\[
\varphi_t \;=\; g_t(S_0,\dots,S_{t-1}).
\]
Although a predictable $\varphi_t$ is also $\mathcal F_t$-measurable (since $\mathcal F_{t-1}\subseteq \mathcal F_t$), the \emph{restriction} is that $\varphi_t$ must already be determined by time $t-1$, so it cannot depend on the new information revealed at time $t$ (e.g.\ on $S_t$).
By contrast, the price $S_t$ is typically $\mathcal F_t$-measurable but not $\mathcal F_{t-1}$-measurable because the time-$t$ price incorporates new randomness/information that was not available at $t-1$.
